...
const WeeklyCalendar: React.FC<WeeklyCalendarProps> = ({ calendarData, selectedLocation }) => {
  ...
  return (
    <div
      className="rounded-[8px] bg-white border border-black w-full max-w-full calendar-main-area"
      style={{
    ...
    >
      <table className="min-w-full border-separate border-spacing-0 font-medium select-none">
        <thead>
          <tr>
            <th className="w-20 h-14 pr-1 align-middle"></th>
            {weekDates.map((d, idx) => (
              <th
                key={idx}
        ...
        <tbody>
          {TIMES.map((time, rowIdx) => (
            <tr key={time}>
              <th
                className="py-0 pr-2 text-right align-middle text-base font-jost font-normal text-black"
