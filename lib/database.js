...
export class DatabaseService {
  ...
  // Comprehensive cleanup: Remove historical data and clear specific dates
  async comprehensiveCleanup(datesToScrape, provider = null, keepDaysOld = 7) {
    try {
      console.log('🧹 Starting comprehensive database cleanup...')
      
      // 1. Remove historical data older than keepDaysOld
      const cutoffDate = new Date()
      ...
      const { data: oldData, error: oldError } = await deleteQuery
      ...
      console.log(`✅ Deleted historical slots older than ${cutoffDateStr}${provider ? ` for ${provider}` : ''}`)

      // 2. Clear slots for the specific dates we're about to scrape
      if (datesToScrape && datesToScrape.length > 0) {
        let clearQuery = this.supabase
          .from('tennis_slots')
          .delete()
          .in('date', datesToScrape)

        if (provider) {
          clearQuery = clearQuery.eq('provider', provider)
        }

        const { data: clearData, error: clearError } = await clearQuery

        if (clearError) {
          console.error('Error clearing slots for scraping dates:', clearError)
          return { success: false, error: clearError.message }
        }

        console.log(`✅ Cleared slots for ${datesToScrape.length} dates${provider ? ` for ${provider}` : ''}`)
      }

      // 3. Get stats after cleanup
      ...
      return { 
        success: true, 
        historicalDeleted: oldData?.length || 0,
        datesCleared: datesToScrape?.length || 0,
        totalSlots: stats.success ? stats.totalSlots : 'unknown'
      }
    } catch (error) {
      console.error('Error in comprehensiveCleanup:', error)
      return { success: false, error: error.message }
    }
  }
